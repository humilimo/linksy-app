...
function EditBioModal(props) {
	const [newBio, setBio] = useState<string | null>(null);

	const submitNewBio = async (e) =>{
		e.preventDefault();
		if(newBio) {
			...
			.then(() => {
				props.setShowEditBioModal(false);
	...
	return (
		...
		<div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
			<form onSubmit={submitNewBio} className="relative w-auto my-6 mx-auto max-w-3xl">
				<div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
					...
					<div className="relative p-6 flex-auto">
						...
						<input name='bio' id='bio' onChange={(e)=>{setBio(e.target.value)}} type="text" className='rounded shadow border border-gray-200 px-1 py-2 mt-3 w-full' />
					</div>
					{/* FOOTER */}
